...
import { AlertCircle, ArrowRight, Bell, CheckCircle2, ChevronLeft, FileText, HeartHandshake, Loader2, LockKeyhole, MessageCircle, ShieldCheck, UploadCloud, UserRound, Video } from "lucide-react";
...
export function SupportPage() {
  ...
  return <AuthRequired><PageIntro eyebrow="Private support" title="Talk to the Givethra team." copy="Use this private conversation for help with your profile, verification or case workflow. Replies appear here and in your notifications." /><div className="rounded-3xl border border-stone-200 bg-white"><div className="max-h-[480px] min-h-80 space-y-3 overflow-auto p-6">{list.data?.length ? list.data.map(message => <div key={message.id} className={`max-w-[82%] rounded-2xl p-4 text-sm leading-6 ${message.senderRole === "admin" ? "bg-emerald-950 text-white" : "ml-auto bg-stone-100 text-slate-700"}`}><p>{message.body}</p><span className={`mt-2 block text-[11px] ${message.senderRole === "admin" ? "text-emerald-100/70" : "text-slate-400"}`}>{message.senderRole === "admin" ? "Givethra support" : "You"} · {new Date(message.createdAt).toLocaleString()}</span></div>) : <div className="grid min-h-64 place-items-center text-center"><MessageCircle className="h-8 w-8 text-emerald-700" /><p className="mt-3 text-sm text-slate-600">Start a private message whenever you need guidance.</p></div>}</div><form onSubmit={onSend} className="border-t border-stone-200 p-4 sm:flex sm:gap-3"><textarea value={body} onChange={e => setBody(e.target.value)} placeholder="Write a message to support…" className="min-h-24 w-full rounded-2xl border border-stone-300 px-4 py-3 text-sm outline-none focus:border-emerald-700 sm:min-h-12" /><button disabled={send.isPending} className="mt-3 rounded-full bg-emerald-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-800 disabled:opacity-60 sm:mt-0">Send</button></form></div></AuthRequired>;
}
