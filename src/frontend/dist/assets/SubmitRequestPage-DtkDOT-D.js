import { c as createLucideIcon, o as ue, u as useAuth, f as useNavigate, r as reactExports, W as getCategoryOffer, X as getOfferClaimCount, S as getCasesByUser, U as getFeedbacks, g as getKycStatus, b as getWallet, Y as getUserSettings, Z as getCaseCounts, j as jsxRuntimeExports, L as Link, _ as Wallet, T as getUserSuspension, $ as upsertUserSuspension, V as insertCaseSubmission, a0 as insertOfferClaim, a1 as updateCategoryOfferUsage, N as uploadFileToStorage } from "./main-g9K_ERoM.js";
...
const CATEGORY_LIMITS = {
  ...
  "Food & Groceries": { type: "fixed", amount: 6e3, label: "Fixed Rs 6,000 per family" },
...
function SubmitRequestPage() {
  ...
  const videoTimerRef = reactExports.useRef(0);
  const videoStartTimeRef = reactExports.useRef(null);
  ...
  async function startVideoRecording() {
    try {
      ...
      recorder.onstop = async () => {
        const wallClockSeconds = videoStartTimeRef.current ? Math.floor((Date.now() - videoStartTimeRef.current) / 1e3) : 0;
        ...
        try {
          actualDuration = await new Promise((resolve) => {
            ...
            v.preload = "metadata";
            const timeout = setTimeout(() => resolve(estimate), 3e3);
            v.onloadedmetadata = () => {
        ...
        if (actualDuration < 60) {
          ue.error(`Please record at least 60 seconds. Current: ${actualDuration}s`);
        ...
        try {
          ...
          setVideoUrl(url);
          ue.success(`Video uploaded successfully! (${fileSizeMB.toFixed(1)}MB, ${actualDuration}s)`);
        } catch (err) {
  ...
  function stopVideoRecording() {
    if (videoStartTimeRef.current) {
      const wallClockSeconds = Math.floor((Date.now() - videoStartTimeRef.current) / 1e3);
