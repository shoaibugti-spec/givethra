...
export default function MedicinesForm({ formData, setFormData, onNext, onBack, isFirst, isLast }: any) {
  ...
  const filteredPharmacies = (HEALTH_INSTITUTES || [])
    .filter((n: string) => n.toLowerCase().includes(search.toLowerCase()))
    .slice(0, 8);

  const hasPharmacy = !!catFields.pharmacy_name || (isOther && !!otherName.trim());
    ...
    >
      <div className="space-y-4">
        <TextInput
          ...
          onChange={(v: string) => setField("patient_name", v)}
        ...
        <TextInput
          ...
          onChange={(v: string) => setField("illness", v)}
          ...
          <div className="space-y-2">
            ...
            {search.length >= 2 && (
              <div className="rounded-xl border divide-y overflow-hidden max-h-48 overflow-y-auto">
                {filteredPharmacies.map((n: string) => (
                  <button
                    key={n}
                    type="button"
        ...
        {(catFields.pharmacy_name || isOther) && (
          <>
            ...
            <FileUpload
              ...
              onUpload={(url: string) => setDoc("doctor_report", url)}
