...
module {
  ...
  // ── OldActor: exactly NewActor from 20260612_000000_AddDepositRequests.mo ─
  type OldActor = {
  ...
  // ── NewActor: adds callerPrincipalIndex ───────────────────────────────────
  type NewActor = {
    accessControlState   : AccessControl.AccessControlState;
    users                : Map.Map<UserId, User>;
    heroStats            : Map.Map<UserId, HeroStats>;
    ...
    depositRequests      : Map.Map<Text, DepositRequest>;
    depositState         : DepositState;
    // New: maps caller principal text → userId for authenticated endpoint lookups
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      callerPrincipalIndex = Map.empty<Text, UserId>();
    };
  };
};
