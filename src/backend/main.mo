...
import DepositsMixin "mixins/deposits-api";
...
actor {
  ...
  // ── Deposits ───────────────────────────────────────────────────────────────
  let depositRequests : Map.Map<Text, DepositsT.DepositRequest>;
  let depositState    : { var nextDepositId : Nat };
  ...
  let kycSubmissions       : Map.Map<Common.UserId, AuthT.KycSubmission>;
  let activeSessions       : Map.Map<Common.UserId, [UsersT.ActiveSession]>;
  // Maps caller principal (caller.toText()) → userId ("usr_N") for auth lookups
  let callerPrincipalIndex : Map.Map<Text, Common.UserId>;
