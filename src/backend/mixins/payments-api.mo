...
mixin (
  ...
  /// Get Support Credit balance for a user by userId (no caller check — for anonymous actor use)
  public query func getWalletByUserId(userId : Text) : async Int {
  ...
  /// Purchase Support Credits via direct IC Ledger deposit (simulated).
  /// Adds `amount` credits to the caller's wallet without requiring Stripe.
  /// Returns ok("Success") on success, err("Unauthorized") if anonymous.
  public shared ({ caller }) func purchaseCreditsDirect(
    amount : Nat,
  ) : async Result.Result<Text, Text> {
    // Reject anonymous callers (anonymous principal = "2vxsx-fae")
    ...
    PaymentLib.addSupportCredits(
      ...
      amount,
      #Purchase,
      ?("Internet Computer Ledger deposit"),
    );
    #ok("Success");
  };
