...
import Nat32 "mo:core/Nat32";import List "mo:core/List";
...
mixin (
  ...
  /// Append a new ActiveSession for a user after login
  func addSession(userId : Common.UserId, deviceHint : Text) {
    let now  = Time.now();
    let sess : UsersT.ActiveSession = {
      id         = "sess_" # Int.abs(now).toText();
      deviceName = if (deviceHint.size() > 0) { deviceHint } else { "Web Browser" };
      os         = "Unknown OS";
    ...
    let existing = switch (activeSessions.get(userId)) {
      case (?arr) arr;
      case null   [];
    };
    activeSessions.add(userId, existing.concat([sess]));
  };
  ...
  /// Deterministic password hash (FNV-1a variant over UTF-8 code points)
  func hashPassword(password : Text) : Text {
    var h : Nat32 = 2_166_136_261;
    for (ch in password.toIter()) {
      let code = ch.toNat32();
      h := (h ^ code) *% 16_777_619;
    };
    h.toNat().toText();
  ...
  public shared ({ caller }) func registerWithGoogle(
    ...
    initStats(userId);
    // Record caller principal → userId so authenticated endpoints work immediately after registration
    callerPrincipalIndex.add(caller.toText(), userId);
    #ok(userToPublic(user));
  };

  public shared ({ caller }) func loginWithGoogle(
    googleId : Text,
  ) : async { #ok : UsersT.UserPublic; #err : Text } {
    switch (authGoogleIndex.get(googleId)) {
      case (?userId) {
        switch (users.get(userId)) {
          case (?user) {
            ...
            addSession(userId, "Google Sign-In");
            // Record caller principal → userId so authenticated endpoints can look up the user
            callerPrincipalIndex.add(caller.toText(), userId);
            #ok(userToPublic(user));
          };
  ...
  public shared ({ caller }) func verifyPhoneOtp(
    phoneNumber : Text,
    otpCode     : Text,
  ) : async { #ok : UsersT.UserPublic; #err : Text } {
    switch (authPhoneIndex.get(phoneNumber)) {
  ...
  public shared func registerWithEmail(
    ...
    let user         = makeUser(userId, fullName, ?email, null, #email, null, ?passwordHash, false, false);
  ...
  public shared ({ caller }) func loginWithEmail(
    email    : Text,
    password : Text,
  ) : async { #ok : UsersT.UserPublic; #err : Text } {
    switch (authEmailIndex.get(email)) {
        ...
        switch (users.get(userId)) {
          case (?user) {
            ...
            switch (user.passwordHash) {
              case (?storedHash) {
                if (storedHash != hashPassword(password)) { return #err("Invalid password") };
                user.lastLoginAt := Time.now();
                addSession(userId, "Email Sign-In");
                callerPrincipalIndex.add(caller.toText(), userId);
                #ok(userToPublic(user));
              };
  ...
  /// Remove a single login device from the caller's device list
  public shared ({ caller }) func revokeDevice(
    ...
    switch (users.get(userId)) {
      case (?user) {
        let owned = user.loginDevices.find(func(d : UsersT.LoginDevice) : Bool { d.id == deviceId }) != null;
        if (not owned) { return #err("Device not found or does not belong to caller") };
        user.loginDevices := user.loginDevices.filter(func(d : UsersT.LoginDevice) : Bool { d.id != deviceId });
  ...
  /// Submit KYC documents — stores submission with new schema and sets kycStatus to #Pending
  public shared ({ caller }) func submitKyc(
    ...
    switch (users.get(userId)) {
      case (?user) {
        if (not user.isActive) { return #err("Account suspended") };
        let submission : AuthT.KycSubmission = {
          documentType;
          documentFileUrl;
          liveVideoUrl;
          status      = #pending;
        ...
        kycSubmissions.add(userId, submission);
  ...
  /// Get the caller's KYC submission (if any)
  public query ({ caller }) func getMyKycSubmission() : async ?AuthT.KycSubmission {
    let userId = switch (callerPrincipalIndex.get(caller.toText())) {
      case (?uid) uid;
